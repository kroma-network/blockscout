  ...
  switch (action.type) {
    ...
    case "RECEIVED_NEW_TRANSACTION_BATCH": {
      ...
      if (transactionsLength < BATCH_THRESHOLD) {
        ...
        return Object.assign({}, state, {
          transactions: [...action.msgs.reverse(), ...state.transactions.slice(0, -1 * action.msgs.length)],
          transactionCount,
        });
      } else {
        return Object.assign({}, state, {
          transactionsBatch: [...action.msgs.reverse(), ...state.transactionsBatch],
          transactionCount,
        });
      }
    }
...
const elements = {
  '[data-chart="historyChart"]': {
    ...
    render(_$el, state, oldState) {
      if (
        !chart ||
        (oldState.availableSupply === state.availableSupply && oldState.marketHistoryData === state.marketHistoryData) ||
